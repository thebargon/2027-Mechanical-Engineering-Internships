...
export const requestContext = new AsyncLocalStorage<{ failures: number }>();
let nextRequestAt = 0;
const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

// Space request starts globally, including retries; callers also use a bounded worker pool.
export async function request(url: string, format: "json" | "text"): Promise<any | null> {
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: { "User-Agent": "MechanicalInternshipTracker/1.0", Accept: format === "json" ? "application/json" : "text/html" },
      });
